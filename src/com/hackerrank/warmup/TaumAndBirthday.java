...
public class TaumAndBirthday {
	...
	public static void main(String[] args) throws IOException {
		...
		while (T > 0) {
			...
			int b = Integer.parseInt(line1.split(" ")[0]);
			int w = Integer.parseInt(line1.split(" ")[1]);
			String line2 = br.readLine();
			int x = Integer.parseInt(line2.split(" ")[0]);
			int y = Integer.parseInt(line2.split(" ")[1]);
			int z = Integer.parseInt(line2.split(" ")[2]);
			BigInteger amount = new BigInteger("0");
			if (x == y) {
				// amount = new BigInteger(String.valueOf(b * x + w * y));
				amount = new BigInteger(String.valueOf(b)).multiply(
						new BigInteger(String.valueOf(x))).add(
						new BigInteger(String.valueOf(w))
								.multiply(new BigInteger(String.valueOf(y))));
			} else if (x > y) {
				if (z > x) {
					amount = new BigInteger(String.valueOf(b))
							.multiply(new BigInteger(String.valueOf(x)))
							.add(new BigInteger(String.valueOf(w))
									.multiply(new BigInteger(String.valueOf(y))));
				} else {
					if ((y + z) < x) {
						amount = new BigInteger(String.valueOf(b))
