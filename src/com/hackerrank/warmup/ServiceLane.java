 ...
 * 
 * Output Format For each test case, print the number that represents the
 * largest vehicle type that can pass through the service lane.
 * 
 * Note Calvin has to pass through all segments from index i to indexj (both
 * inclusive).
 * 
 * Constraints 2 <= N <= 100000 1 <= T <= 1000 0 <= i < j < N 2 <= j-i+1 <=
 * min(N,1000) 1 <= width[k] <= 3, where 0 <= k < N
 * 
 * Sample Input #00
 * 
 * 8 5 2 3 1 2 3 2 3 3 0 3 4 6 6 7 3 5 0 7 Sample Output #00
 * 
 * 1 2 3 2 1 Explanation for Sample Case #0 Below is the representation of lane.
 * 
 * |HIGHWAY|Lane| -> Width
 * 
 * 0: | |--| 2 1: | |---| 3 2: | |-| 1 3: | |--| 2 4: | |---| 3 5: | |--| 2 6: |
 * |---| 3 7: | |---| 3 (0, 3): Because width[2] = 1, only the bike represented
 * as 1 can pass through it. (4, 6): Here the largest allowed vehicle which can
 * pass through the 5th segment is car and for the 4th and 6th segment it's the
 * truck. Hence the largest vehicle allowed in these segments is a car. (6, 7):
 * In this example, the vehicle enters at the 6th segment and exits at the 7th
 * segment. Both segments allow even truck to pass through them. Hence truck is
 * the answer. (3, 5): width[3] = width[5] = 2. While 4th segment allow the
 * truck, 3rd and 5th allow upto car. So 2 will be the answer here. (0, 7): Bike
 * is the only vehicle which can pass through the 2nd segment, which limits the
...
public class ServiceLane {
	...
	public static void main(String[] args) {
		...
		for (int j = 0; j < T; j++) {
			...
			int result = 0;
			result = getMin(arr, a, b);
			System.out.println(result);
		}
		scan.close();
	}

	public static int getMin(int arr[], int a, int b) {
