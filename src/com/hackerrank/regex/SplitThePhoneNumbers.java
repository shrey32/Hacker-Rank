...
public class SplitThePhoneNumbers {
	...
	public static void main(String[] args) throws IOException {
		...
		while (T > 0) {
			...
			splitThePhoneNumbers(number);
			T--;
		}
		br.close();
	}

	private static void splitThePhoneNumbers(String number) {
		String[] numbers = null;
		if (number.contains(" ")) {
		...
		if (numbers != null && numbers.length == 3) {
			Pattern pattern1 = Pattern.compile("^[0-9]{1,3}");
			Pattern pattern3 = Pattern.compile("^[0-9]{4,10}$");
			if (pattern1.matcher(numbers[0]).matches()
					&& pattern1.matcher(numbers[1]).matches()
					&& pattern3.matcher(numbers[2]).matches()) {
				System.out.println("CountryCode=" + numbers[0]
						+ ",LocalAreaCode=" + numbers[1] + ",Number="
						+ numbers[2]);
			}
		}
	}
