...
public class IPAddressValidation {
	...
	public static void main(String[] args) throws NumberFormatException,
		...
		while (T > 0) {
			...
			ipAddressValidation(ip.trim());
			T--;
		}
		br.close();
	}

	private static void ipAddressValidation(String ip) {
		Pattern pattern1 = Pattern
				.compile("([01]?[0-9]?[0-9]|2[0-4][0-9]|25[0-5]).([01]?[0-9]?[0-9]|2[0-4][0-9]|25[0-5]).([01]?[0-9]?[0-9]|2[0-4][0-9]|25[0-5]).([01]?[0-9]?[0-9]|2[0-4][0-9]|25[0-5])");
		Pattern pattern2 = Pattern
				.compile("[0-9a-f]{1,4}:[0-9a-f]{1,4}:[0-9a-f]{1,4}:[0-9a-f]{1,4}:[0-9a-f]{1,4}:[0-9a-f]{1,4}:[0-9a-f]{1,4}:[0-9a-f]{1,4}");
		if (pattern1.matcher(ip).matches()) {
			System.out.println("IPv4");
		} else if (pattern2.matcher(ip).matches()) {
